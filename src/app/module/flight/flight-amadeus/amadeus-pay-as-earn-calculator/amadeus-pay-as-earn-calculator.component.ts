...
@Component({
  selector: 'app-amadeus-pay-as-earn-calculator',
  templateUrl: './amadeus-pay-as-earn-calculator.component.html',
  styleUrls: ['../../flight-result-body/modal/pay-as-earn-calculator/pay-as-earn-calculator.component.scss']
})
export class AmadeusPayAsEarnCalculatorComponent {
  ...
  inputValueChanges(event: any) {
    this.installmentAmount = this.sharedService.getInstallmentAmount(this.BookedFlightData?.price?.grandTotal ?? 0, ( event.value < 20 ? 20 : event.value > 100  ? 100 : event.value));
  ...
  calclateInstallMentData(percentage: any, installmentsTimes:number) {
    const defaultValue = this.BookedFlightData?.price?.grandTotal ?? 0;
...
payAsYouAmount(showType: boolean = true){
  ...
  let flightFareInstallementDetails= {
    dateOfDeparture: this.BookedFlightData?.itineraries[0]?.segments[0]?.departure?.at,
    totalFare: this.BookedFlightData?.price?.grandTotal,
    currency: 'NGN',
